...
// Cached Community Event operations
const communityEventCacheKey = {
  all: () => 'community_events:all',
  byId: (id: number) => `community_event:${id}`
};

export const getCommunityEventsCached = async (): Promise<any[]> => {
  const cacheKey = communityEventCacheKey.all();

  let events = await getCache<any[]>(cacheKey);

  if (!events) {
    events = await db.getCommunityEvents();
    await setCache(cacheKey, events, LIST_TTL);
  }
...
export const getCommunityEventByIdCached = async (id: number): Promise<any | null> => {
  const cacheKey = communityEventCacheKey.byId(id);

  let event = await getCache<any>(cacheKey);

  if (!event) {
    event = await db.getCommunityEventById(id);
    if (event) {
      await setCache(cacheKey, event, ITEM_TTL);
    }
  }
...
export const createCommunityEventCached = async (event: {
  user_id: number;
  title: string;
  description?: string;
  date: string;
  ...
  const newEvent = await db.createCommunityEvent(event);

  // Invalidate community events list cache
  await deleteCache(communityEventCacheKey.all());

  // Cache the new event
  await setCache(communityEventCacheKey.byId(newEvent.id), newEvent, ITEM_TTL);

  return newEvent;
};

export const updateCommunityEventCached = async (id: number, userId: number, updates: {
  title?: string;
  description?: string;
  date?: string;
  location?: string;
  ...
  const updatedEvent = await db.updateCommunityEvent(id, userId, updates);

  if (updatedEvent) {
    // Invalidate related caches
    await deleteCache(communityEventCacheKey.all());
    await deleteCache(communityEventCacheKey.byId(id));

    // Cache the updated event
    await setCache(communityEventCacheKey.byId(updatedEvent.id), updatedEvent, ITEM_TTL);
  }

  return updatedEvent;
};

export const deleteCommunityEventCached = async (id: number, userId: number): Promise<boolean> => {
  const result = await db.deleteCommunityEvent(id, userId);
